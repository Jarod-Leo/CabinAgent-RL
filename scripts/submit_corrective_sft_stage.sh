...
gate_report="${DIRECT_RL_GATE_REPORT:-reports/direct_rl_gate_G03_132967.json}"
...
case "$stage" in
  smoke)
    base_run_id="sft_corrective_smoke_${pipeline_id}"
    job_name="car-f02-smoke"
    walltime="02:00:00"
    next_stage="full"
    ...
    ;;
  full)
    base_run_id="sft_corrective_full_${pipeline_id}"
    job_name="car-f02"
    walltime="08:00:00"
    next_stage=""
    ...
    ;;
  *)
    echo "Unknown corrective SFT stage: $stage" >&2
    exit 2
    ;;
esac
...
exports="ALL,PROJECT_ROOT=$PROJECT_ROOT,RUN_ID=$run_id,PIPELINE_ID=$pipeline_id,DIRECT_RL_GATE_REPORT=$gate_report,SFT_CONFIG=$config,NEXT_SFT_STAGE=$next_stage,POST_SFT_ACTION=$post_sft_action,SFT_STAGE_SUBMITTER=scripts/submit_corrective_sft_stage.sh,POST_GATE_LABEL=G04"
...
[[ -n "$job_id" ]] || { echo "Empty Slurm job ID for corrective SFT $stage" >&2; exit 1; }
...
echo "CORRECTIVE_SFT_SUBMITTED stage=$stage job_id=$job_id run_id=$run_id"
